...
| 공정 | 어떻게 정해지나 |
|---|---|
| **제작** | 자동 — 덱 HTML이 없으면 미착수, 있으면 `작업중`, `CONFIRMED.md`에 있거나 소스 `상태`에 확정이 있으면 `완료` |
...
| `해당없음` (`—`) | 그 공정이 애초에 없다. 실습 클립의 제작·대본 |
...
`슬라이드: false`인 클립은 화면 시연이 본체라 덱·대본을 만들지 않는다. 제작·대본이 `해당없음`으로
빠지고 촬영만 추적한다. 커리큘럼 72클립이 전부 표에 들어가므로 실습 클립도 촬영 진행이 보인다.
